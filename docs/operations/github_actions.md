...
- **Steps per band** (parallel matrix):
  ...
  7. Run backtests and save aggregate accuracy (skippable via `skip_accuracy`; Notebook uses `--shows 50`, Deal uses `--shows 10`; emits `backtest_incremental_all_scored` output)
