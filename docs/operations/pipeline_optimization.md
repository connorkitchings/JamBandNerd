...
- `scripts/sync_retained_prediction_corpus.py`: A corpus sync wrapper that scores each band's registered model version against the retained last-50 eligible completed shows and prunes older derived rows plus stale model-version rows.
