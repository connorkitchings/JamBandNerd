...
class WSPCollector(BandCollector):
    '''Collects WSP data by scraping everydaycompanion.com.'''
    ...
    def _get_existing_show_urls(self) -> List[str]:
        ...
        try:
            response = self.supabase_client.table("wsp_shows_raw").select("source_url").execute()
            return [item['source_url'] for item in response.data if item.get('source_url')]
    ...
    def _validate_song_name(self, song_name: str) -> bool:
        ...
        if len(song_name) > 80:
            logger.debug(f"Rejecting long song name (>{len(song_name)} chars): {song_name[:50]}...")
            return False
            
        # Enhanced statistics detection - more comprehensive patterns
        stats_indicators = [
            "Song Stats", "LTP Date", "L3TP", "#/10", "#/100", "#/Ever", 
            "StatsSong", "Last Time Played", "Average of last",
            "LTPL3TP", "DateLTP", "LTP (Last Time Played)",
            "Number of shows since", "Number of times played",
            "Total number of times", "Average of last 3",
            # Common contamination patterns from actual data
            "ALONE 12/31/23", "BEAR 03/24/24", "APLANE 03/22/24"
        ]
        ...
        if song_name.startswith(("01/", "02/", "03/", "04/", "05/", "06/", 
                                "07/", "08/", "09/", "10/", "11/", "12/")):
    ...
    def _parse_setlist_from_text(self, soup: BeautifulSoup, show_id: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if ':' in line and (line.startswith('0:') or line.startswith('1:') or line.startswith('2:') or 
                               line.startswith('3:') or line.startswith('E:') or line.startswith('Encore')):
                
                # Extract set number
                if line.startswith('E:') or line.startswith('Encore'):
                    set_number = 'E'
                    songs_part = line[2:].strip() if line.startswith('E:') else line[6:].strip()
    ...
    def _scrape_single_setlist(self, show_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        '''Scrapes the setlist for a single show with improved parsing.'''
        ...
        try:
            ...
            if final_url != show_url:
                ...
                # Note: We don't update the database here, just use the working URL
            ...
            if setlist_data:
                ...
                valid_songs = [s for s in setlist_data if self._validate_song_name(s['song_name'])]
                contaminated_count = len(setlist_data) - len(valid_songs)
                
                if contaminated_count > 0:
                    logger.warning(f"Filtered {contaminated_count} contaminated songs from {show_url}")
                
                if valid_songs:
                    return valid_songs
                else:
                    logger.warning(f"All songs filtered as contaminated for {show_url}, trying fallback parsing")
    ...
    def collect_shows(self, start_date: Optional[date] = None, end_date: Optional[date] = None) -> List[Dict[str, Any]]:
        """Collects show data by scraping the tour pages (tourYY.asp)."""
        shows = []
        ...
        if start_date and end_date:
            ...
            if start_date.year < 2000 <= end_date.year:
                years_to_scrape = list(range(start_year_2d, 100)) + list(range(0, end_year_2d + 1))
            elif start_date.year >= 2000 and end_date.year >= 2000:
                years_to_scrape = list(range(start_year_2d, end_year_2d + 1))
            else:
                years_to_scrape = list(range(start_year_2d, end_year_2d + 1))
                
            logger.info(f"Collecting shows for years: {start_date.year}-{end_date.year} (2-digit: {years_to_scrape})")
        else:
            # Default: scan all historical years
            years_to_scrape = list(range(86, 100)) + list(range(0, current_year_2_digit + 1))
            logger.info(f"No date range specified, collecting all historical shows")

        iterable = tqdm(years_to_scrape, desc=f"Collecting {self.ARTIST_NAME} shows", unit="year")
        ...
        for year_2d in iterable:
            ...
            try:
                ...
                def find_show_table(tag):
                    ...
                    return tag.find('a', href=lambda href: href and ('.asp' in href and ('setlist' in href or '/setlists/' in href)))
                ...
                setlist_links = target_table.find_all('a', href=lambda href: href and ('.asp' in href and ('setlist' in href or '/setlists/' in href)))
                ...
                for link in setlist_links:
                    try:
                        ...
                        try:
                            ...
                            show_date = datetime.strptime(f"{month}/{day}/{full_year}", "%m/%d/%Y").date()
                        except (ValueError, IndexError) as date_error:
                            logger.debug(f"Skipping link due to date parse error: {link_text} | Error: {date_error}")
                            continue
                        
                        # Parse venue and location
                        # Format: "Venue Name, City, State" or "Venue Name, City State"
                        venue_parts = [part.strip() for part in venue_location_part.split(',')]
                        ...
                        if len(venue_parts) >= 3:
                            ...
                            city_state_parts = city_state.rsplit(' ', 1)  # Split from right to handle multi-word cities
                        ...
                        logger.debug(f"Skipping link due to parse error in year {year_str}: {link_text} | Error: {parse_error}")
                        continue
    ...
    def collect_setlists(self, shows_to_process: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_show = {executor.submit(self._scrape_single_setlist, show): show for show in shows_to_process}
            iterable = tqdm(future_to_show, total=len(shows_to_process), desc=f"Collecting {self.ARTIST_NAME} setlists")
    ...
    def collect_songs(self) -> List[Dict[str, Any]]:
        '''Collects song data by scraping the song catalog page.'''
        ...
        try:
            ...
            songs_df.columns = ["code", "song_name", "first_played", "last_played", "times_played", "aka"]
    ...
    def collect_venues(self) -> List[Dict[str, Any]]:
        ...
        logger.warning(f"⚠️ {self.ARTIST_NAME}: Venue collection is not separately implemented. Venues are collected as part of shows.")
        return []
