...
def clean_song_name_for_display(name: str, band: str) -> str:
...
def format_show_header(show_details: dict[str, Any]) -> str:
...
def build_prediction_lookup(predictions_df: pd.DataFrame, band: str) -> dict[str, int]:
    ...
    for idx, row in predictions_df.iterrows():
        normalized = clean_song_name_for_display(str(row["song_name"]), band).lower()
        if not normalized:
            continue
...
def compute_summary(
    ...
    for song_name in unique_song_names:
        cleaned = clean_song_name_for_display(song_name, band)
...
def render_hero(
    ...
    st.markdown(
        ...
        <div class="jbn-card jbn-hero">
            <div class="jbn-hero__title">Show Context</div>
...
def render_setlist(
    ...
    for set_num in set_numbers:
        ...
        for _, song_row in set_data.iterrows():
            song_name_clean = clean_song_name_for_display(song_row["song_name"], band)
            ...
            for chunk in _chunked(list(prior_ids), 500):
                ...
                for row in resp.data or []:
                    ...
                    if sid is None or str(sid) not in prior_ids:
                        continue
                    cleaned = clean_song_name_for_display(row.get("song_name"), band)
                    if cleaned:
