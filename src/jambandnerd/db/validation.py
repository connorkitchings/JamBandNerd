...
# Columns that are known to be auto-generated by the database and therefore
# should not be required in incoming DataFrames during validation.
AUTO_GENERATED_COLUMNS = {"id", "song_id", "show_id", "created_at", "updated_at"}
...
def _coerce_series(series: pd.Series, expected: str) -> pd.Series:
    ...
    if expected == "timestamp":
        ...
        ts = pd.to_datetime(series, errors="coerce", format="ISO8601", utc=True)
        if not ts.notna().any():
...
def coerce_df_types(
    df: pd.DataFrame, schema_rows: list[Mapping[str, Any]]
) -> pd.DataFrame:
    ...
    schema = {
        str(r.get("column_name")): _pg_type_to_expected(str(r.get("data_type", "text")))
...
def validate_dataframe_against_table(
    ...
    col_nullable = {
        str(r.get("column_name")): str(r.get("is_nullable", "YES")).upper() == "YES"
        for r in schema_rows
    }
    ...
    schema_cols_no_pk = [c for c in schema_cols if c not in AUTO_GENERATED_COLUMNS]
    ...
    for c in df_cols:
        ...
        if expected in ("integer", "numeric") and observed in (
            "integer",
            "numeric",
            "text",
        ):
        ...
        if expected in ("date", "timestamp") and observed in (
            "date",
            "timestamp",
            "text",
        ):
    ...
    for c, nullable in col_nullable.items():
        ...
        if c in AUTO_GENERATED_COLUMNS or c in ("created_at", "updated_at"):
            continue
