...
"""Generate Widespread Panic predictions using the CK+ model."""
...
def main(date_str: str | None) -> None:
    """Generate and save CK+ predictions for Widespread Panic."""
    ...
    print(f"Generating Widespread Panic CK+ predictions for reference date: {reference_date.isoformat()}")

    model_data = generate_model_data(shows_df, setlists_df, reference_date)

    # Adjusted parameters for WSP's larger catalog
    predictor = CKPlusPredictor(retired_gap_threshold=150)  # Using the same as Phish for now
    ...
    if predictions_list:
        print("Top 25 CK+ predictions (Widespread Panic):")
    ...
    print("Successfully saved Widespread Panic CK+ predictions.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Widespread Panic CK+ predictions.")
