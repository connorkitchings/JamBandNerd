"""Backtest WSP notebook model over historical shows and save per-show accuracy."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Backtest WSP notebook model and save per-show accuracy")
    ...
    print(f"Backtesting WSP on {len(ref_dates)} show dates from {start_d} to {end_d}")
