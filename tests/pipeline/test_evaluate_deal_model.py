...
def test_generate_report_emits_registry_aligned_promotion_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        module, "generate_model_data", lambda *_args, **_kwargs: object()
    )
    monkeypatch.setattr(
        module, "build_evaluation_predictor", lambda slug, band: _DealPredictorStub()
    )
    monkeypatch.setattr(
        module,
    ...
    assert set(report["comparison_window"]["metrics"].keys()) == {
        "deal",
        "ckplus",
        "notebook",
    }
