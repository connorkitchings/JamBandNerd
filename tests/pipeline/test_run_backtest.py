...
class _NotebookPredictorStub:
    def predict(self, model_data, top_k=50):  # noqa: ARG002
        return ([_Prediction("Song A"), _Prediction("Song B"), _Prediction("Song C")], {})
...
def test_run_backtest_persists_string_show_ids(monkeypatch):
    ...
    def capture_upsert(table_name: str, df: pd.DataFrame, conflict_columns):  # noqa: ANN001
        captured["table_name"] = table_name
        captured["df"] = df.copy()
        captured["conflict_columns"] = list(conflict_columns)

    monkeypatch.setattr(run_backtest_module, "fetch_table", fetch_table)
    monkeypatch.setattr(run_backtest_module, "generate_model_data", lambda *args, **kwargs: object())
    monkeypatch.setattr(run_backtest_module, "NotebookPredictor", _NotebookPredictorStub)
