...
```mermaid
graph TD
    A[Data Sources] -->|phish.net, elgoose.net, scraping| B[Transformation Pipeline]
    B -->|Standardized Data| C[Supabase DB]
    C -->|Query| D[Web Interface]
    C -->|Train| E[Prediction Models]
    E -->|Predictions| C
    F[GitHub Actions] -->|Trigger| B
    F -->|Monitor| E
    D -->|Display| G[User]
    E -->|Update| D
...
- **Output**: Raw data tables in Supabase
- **Error Handling**: Continue pipeline with existing data on source failure

#### Transformation Layer

- **Purpose**: Convert raw data to standardized format for modeling
- **Processing**: In-memory transformation (no intermediate tables)
- **Output**: Standardized data structures passed directly to models
...
**Next Song Predictions**: `{band}_predictions`

```sql
CREATE TABLE phish_predictions (
    id SERIAL PRIMARY KEY,
    show_id VARCHAR(50),
    set_number INTEGER,
    ...
    created_at TIMESTAMP DEFAULT NOW(),
    INDEX (show_id, set_number, song_position),
    INDEX (model_name, created_at)
);
```

**Accuracy Tracking**: `{band}_accuracy`

```sql
CREATE TABLE phish_accuracy (
    id SERIAL PRIMARY KEY,
    show_id VARCHAR(50) NOT NULL,
    model_name VARCHAR(50) NOT NULL,
    model_version VARCHAR(20) NOT NULL,
    ...
    UNIQUE (show_id, model_name, model_version)
);
```
