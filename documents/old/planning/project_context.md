...
with robust orchestration and analytics. The architecture is designed to be highly extensible, allowing for the easy addition of new bands and prediction models.
...
and standardized analytics/prediction modules. For detailed pipeline architecture, see [pipeline_architecture.md](../pipeline_architecture.md).
...
| (Phish, Goose, WSP)  |      | (CSV, JSON, Logs) |      | (CK+, Notebook)     |
+--------------------------+      +-------------------+      +---------------------+
```
...
> **Note:** This structure follows data science best practices, with modular separation for data
 collection, processing, modeling, prediction, publishing, and documentation.
> **Note on Data Storage:** The local `data/` directory is deprecated. All data pipelines are being
 ...
 the source of truth.
> **Note:** The `venv` directory, `__pycache__` directories, and `.DS_Store` files are automatically
...
> **Note:** Python 3.12.x is recommended for full compatibility (especially for lxml). All
